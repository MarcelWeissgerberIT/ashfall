...
export default defineConfig({
 ...
 plugins:[{name:'ashfall-pages-assets',enforce:'pre',transform(code,id){if(id.startsWith(root+'app/')&&/\.(tsx?|m?js)$/.test(id))return code.replace(/(['"`(])\/(images|audio)\//g,(_,quote,folder)=>quote+base+folder+'/');}}],
