...
export default function ItemIcon({id,size=24}:{id:string;size?:number}) {
  const cell=itemCells.indexOf(id==='samplecase'?'toolbox':id);if(cell>=0)return <span className="item-art item-atlas" aria-hidden="true" style={{width:size*2,height:size*2,backgroundImage:'url(/images/loot/items-v2.png)',backgroundSize:'500% 400%',backgroundPosition:`${cell%5*25}% ${Math.floor(cell/5)*100/3}%`}}/>;
