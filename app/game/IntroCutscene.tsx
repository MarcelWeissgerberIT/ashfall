...
export default function IntroCutscene({onClose,portalContainer}:{onClose:()=>void;portalContainer?:HTMLElement|null}){
 const video=useRef<HTMLVideoElement>(null);
 const [sound,setSound]=useState(false),[progress,setProgress]=useState(0),[error,setError]=useState(false),[paused,setPaused]=useState(false),[loading,setLoading]=useState(true);
 ...
 <DialogTitle className="sr-only">ASHFALL — A promise to Koda</DialogTitle>
 ...
 <video ref={video} className="intro-film" src="/video/ashfall-prologue-en.mp4" poster={ART.sectors[0].src} autoPlay muted={!sound} playsInline preload="auto"
 ...
 <header className="intro-controls"><span>ASHFALL / PROLOGUE</span><button onClick={onClose}>Skip intro <span>Esc</span></button></header>
 ...
 paused?<section className="intro-film-message"><button onClick={()=>void play(sound)}>Play intro →</button></section>:loading?<p className="intro-film-loading" role="status">Loading film …</p>:null}
