...
export default function IntroCutscene({onClose,portalContainer,kind='intro',audible=false,replay=false}:{replay?:boolean;audible?:boolean;kind?:'intro'|'evac';onClose:()=>void;portalContainer?:HTMLElement|null}){
 ...
 <header className="intro-controls"><span>{kind==='evac'?'ASHFALL / NEXT STOP: HAVEN':'ASHFALL / PROLOGUE'}</span><button onClick={onClose}>{replay?'Back to cinematics':kind==='evac'?'Continue to Haven':'Skip intro'} <span>Esc</span></button></header>
