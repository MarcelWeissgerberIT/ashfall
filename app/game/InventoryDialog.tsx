...
export default function InventoryDialog({game,portalContainer,onCraft}:{game:any;onCraft?:()=>void;portalContainer?:HTMLElement|null}){
 ...
 const selected=selection&&(selection.from==='loot'?container?.contents.includes(selection.id):selection.from==='bag'?game.backpack.includes(selection.id):game.equipment[selection.from]===selection.id)?selection:lootIds.length?{id:lootIds[0],from:'loot'}:ids.length?{id:ids[0],from:'bag'}:null;
 ...
 const card=(id:string,from:string)=>{const def=(ITEMS as any)[id],count=from==='loot'?container.contents.filter((v:string)=>v===id).length:game.backpack.filter((v:string)=>v===id).length,equipped=false;return <div className={`supply-card ${selected?.id===id&&selected.from===from?'selected':''} ${equipped?'is-equipped':''} ${def.kind==='quest'?'quest':''}`} key={id}>
   <button className="supply-select" onPointerDown={e=>begin(e,id,from)} onClick={()=>{if(clicked())setSelection({id,from})}} onDoubleClick={()=>{if(from==='loot'&&clicked())game.takeLoot(id)}} aria-pressed={selected?.id===id&&selected.from===from}><ItemIcon id={id} size={40}/><strong>{def.name}</strong><small>{def.weight.toFixed(1)} kg {count>1?`· ×${count}`:''}</small>{equipped&&<span className="equipped-label"><Check size={10}/> WORN / HELD</span>}</button>
 ...
 return <Localized><Dialog open={open} onOpenChange={v=>{if(!v)game.closeInventory()}}>
  <DialogContent className="inventory-dialog workbench-dialog" showCloseButton={false} portalContainer={portalContainer}>
   <header className="workbench-header"><div><span className="pack-eyebrow">FIELD EQUIPMENT · WORLD PAUSED <LanguageSwitch/>{onCraft&&<button className="language-toggle" onClick={onCraft}>Crafting</button>}</span><DialogTitle>{container?<PackageOpen size={25}/>:<Backpack size={25}/>} {container?container.name:'Mara’s equipment'}</DialogTitle><DialogDescription>{!game.canManageInventory?'Preview only. Enter the sector to change equipment.':container?'Double-click loot to take one. Drag carried items outside this window to drop them.':'Drag gear onto a slot to equip it, or outside this window to drop it. On touchscreens, use the corner grip.'}</DialogDescription></div><button className="pack-close" aria-label="Close inventory and return to game" onClick={()=>game.closeInventory()}><X size={21}/></button></header>
   ...
   <div className="workbench-body">
    <section className="equipment-bay" aria-label="Character and equipment slots">
     ...
     <div className="equipment-slots">{(['hand','body','head'] as const).map(slot=>{const id=game.equipment[slot],def=id?(ITEMS as any)[id]:null;return <div data-dropzone={slot} className={`gear-slot ${zoneClass(slot)}`} key={slot}><button onPointerDown={e=>id&&begin(e,id,slot)} onClick={()=>{if(clicked()&&id)setSelection({id,from:slot})}} aria-label={`${slot} slot: ${def?.name||'empty'}`}>{id?<ItemIcon id={id} size={23}/>:<span aria-hidden="true" style={{width:46,height:46,flexShrink:0}}/>}<span><small>{slot==='hand'?'IN HAND':slot.toUpperCase()}</small><strong>{def?.name||'Drop gear here'}</strong></span></button>{id&&<><button className="gear-drag" onPointerDown={e=>begin(e,id,slot,true)} aria-label={`Drag equipped ${def.name}`}><GripVertical size={15}/></button><button className="gear-remove" disabled={!game.canManageInventory} onClick={()=>game.unequip(slot)} aria-label={`Unequip ${def.name}`}><X size={14}/></button></>}</div>})}</div>
    ...
    <ScrollRegion label="Storage and backpack items" className="supplies-bay">
     ...
     <section className={`bag-shelf ${zoneClass('bag')}`} data-dropzone="bag"><div className="shelf-title"><span><Backpack size={17}/> YOUR BACKPACK <small>{game.backpack.length} items</small></span></div><p>{container?'Drag loot here to collect it.':'Drag equipped gear here to remove it without dropping it.'}</p><div className="supply-grid">{ids.map(id=>card(id,'bag'))}</div>{!ids.length&&<div className="storage-empty"><Backpack size={30}/><strong>Your backpack is empty</strong><span>Pick up supplies or take something from a container.</span></div>}</section>
   ...
   <footer className="item-inspector">{item&&selected?<><ItemIcon id={selected.id} size={30}/><div className="inspector-copy"><small>{kinds[item.kind]} · {item.weight.toFixed(1)} kg</small><strong>{item.name}</strong><p>{item.desc}</p></div><div className="inspector-actions">
    {selected.from==='loot'?<button className="primary-btn" disabled={game.weight+item.weight>CAPACITY+.00001} onClick={()=>game.takeLoot(selected.id)}>TAKE <ArrowRight size={15}/></button>:<>
     {item.slot&&<button className="primary-btn" disabled={!game.canManageInventory} onClick={()=>game.transferItem(selected.id,selected.from,selected.from==='bag'?item.slot:'bag')}>{selected.from==='bag'?'EQUIP':'UNEQUIP'}<Shirt size={15}/></button>}
     ...
     {container&&<button className="secondary-btn" disabled={!game.canManageInventory} onClick={()=>game.transferItem(selected.id,selected.from,'loot')}>STORE</button>}
     <button className="secondary-btn" disabled={!game.canManageInventory} onClick={()=>game.transferItem(selected.id,selected.from,'ground')}>DROP</button>
    </>}
   ...
   {drag&&createPortal(<div className={`drag-ghost ${drag.valid?'valid':''}`} aria-hidden="true" style={{left:drag.x+12,top:drag.y+12}}><ItemIcon id={drag.id} size={27}/><span>{(ITEMS as any)[drag.id].name}{drag.over==='ground'&&drag.valid&&<small style={{display:'block'}}>Drop on the ground</small>}</span></div>,portalContainer||document.body)}
  </DialogContent>
 </Dialog></Localized>;
}
