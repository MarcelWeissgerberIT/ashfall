  ...
  <DialogContent className="inventory-dialog workbench-dialog" showCloseButton={false} portalContainer={portalContainer}>
   ...
   <div className="workbench-body">
    <section className="equipment-bay" aria-label="Character and equipment slots">
     ...
     <div className="equipment-slots">{(['hand','body','head'] as const).map(slot=>{const id=game.equipment[slot],def=id?(ITEMS as any)[id]:null;return <div data-dropzone={slot} className={`gear-slot ${zoneClass(slot)}`} key={slot}><button onPointerDown={e=>id&&begin(e,id,slot)} onClick={()=>{if(clicked()&&id)setSelection({id,from:slot})}} aria-label={`${slot} slot: ${def?.name||'empty'}`}><ItemIcon id={id||(slot==='hand'?'fists':slot==='body'?'jacket':'helmet')} size={23}/><span><small>{slot==='hand'?'IN HAND':slot.toUpperCase()}</small><strong>{def?.name||'Drop gear here'}</strong></span></button>{id&&<><button className="gear-drag" onPointerDown={e=>begin(e,id,slot,true)} aria-label={`Drag equipped ${def.name}`}><GripVertical size={15}/></button><button className="gear-remove" disabled={!game.canManageInventory} onClick={()=>game.unequip(slot)} aria-label={`Unequip ${def.name}`}><X size={14}/></button></>}</div>})}</div>
