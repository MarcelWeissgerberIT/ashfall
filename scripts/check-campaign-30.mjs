...
for(let level=7;level<90;level++){
 ...
 tasks++;
 go(exit.id);assert.equal(game.mode,level===89?'won':'complete');
 console.log(`${level+1}: ${game.data.name} completed, ${Math.ceil(game.health)} HP`);
 if(level<89){game.next();game.start(false)}
}
assert.equal(tasks,166);assert(game.messages.some(m=>m.text.includes('Jonas')));assert(game.messages.some(m=>m.text.includes('Aster')));
...
console.log('90 chapters, 166 generated-chapter tasks, traversal, combat survival, crafting, gating, saved timers and legacy continuation verified.');

for(const level of [29,59]){const legacy=new Game();legacy.loadLevel(level);const raw=JSON.parse(createSave(legacy));raw.state.mode="won";restoreSave(legacy,JSON.stringify(raw));assert.equal(legacy.mode,"complete");legacy.next();assert.equal(legacy.level,level+1);}
