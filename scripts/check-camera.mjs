...
controls.reset();controls.setZoom(3.2);const height=camera.position.y;controls.rotate(1);near(camera.position.y,height);assert(Math.abs(camera.position.x-offset.x)<1e-7);assert(Math.abs(camera.position.z)<1e-7,'45 degree turn rotates the isometric offset');
for(let i=0;i<120;i++)controls.follow({x:5,z:4},{x:7,z:5},1/60);assert(target.distanceTo(new THREE.Vector3(6,0,4.5))<.01,'Camera follows both companions');
controls.setFollowing(false);const free=target.clone();for(let i=0;i<120;i++)controls.follow({x:-5,z:-4},{x:-7,z:-5},1/60);assert(target.equals(free),'Free camera remains stationary');
controls.setFollowing(true);controls.follow({x:-8,z:-7},{x:8,z:7},1/60);assert(camera.zoom<3.2,'Spread out companions fit in view');
controls.reset();near(offset.x,28);near(offset.z,28);fire('keydown',{key:'e'});assert(Math.abs(offset.z)<1e-7);fire('keydown',{key:'q'});near(offset.x,28);near(offset.z,28);
