...
for(const call of calls){const g=new Game();g.loadLevel(call.level);g.start(false);g.time=27;updateStoryEvents(g);assert(!g.messages.some(m=>m.id==='background-'+g.level));g.time=28;if(call.afterObjectives)g.entities.find(e=>e.id==='story-b').done=true;updateStoryEvents(g);assert.equal(g.lastRadio.voiceId,call.id);assert.equal(g.messages.at(-1).sender,call.sender);const n=g.messages.length;updateStoryEvents(g);assert.equal(g.messages.length,n);restoreSave(g,createSave(g));updateStoryEvents(g);assert.equal(g.messages.length,n);for(const lang of ['','de/'])assert(fs.statSync('public/audio/radio/'+lang+call.id+'.mp3').size>10000)}
for(let i=0;i<90;i++){const g=new Game();g.loadLevel(i);updateStoryEvents(g,true);assert(g.messages.some(m=>m.id==='background-'+i));}
console.log('All 90 background events, 180 story voice files, speaker identity, timing and save-safe deduplication verified.');

import briefings from '../app/game/chapter-briefings.json' with {type:'json'};
assert.equal(calls.length,90);assert.equal(briefings.length,90);
...
console.log('90 timed bilingual briefings and save-safe deduplication passed.');
