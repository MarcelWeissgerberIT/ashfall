...
// A complete play performance must settle smoothly and keep paws above the floor.
dog.mode='idle';dog.action=0;delete dog.care;
for(let i=0;i<120;i++)animatePuppy(puppy,dog,1/60,true,i/60);
dog.care={kind:'play',at:Date.now()};let previous=puppy.userData.body.position.clone(),maxJump=0;
for(let i=0;i<360;i++){
 animatePuppy(puppy,dog,1/60,true,i/60);puppy.updateMatrixWorld(true);
 maxJump=Math.max(maxJump,previous.distanceTo(puppy.userData.body.position));previous.copy(puppy.userData.body.position);
 for(const leg of puppy.userData.legs)assert(leg.userData.paw.getWorldPosition(new THREE.Vector3()).y>=.035,'Play paws clear the ground');
}
assert(maxJump<.015,'Play pose has no abrupt frame jumps');assert(puppy.userData.play<.001,'Play settles back to idle');
